...
class ProfileScreen extends StatefulWidget {
  ...
  const ProfileScreen({super.key, this.openedFromAdmin = true});
...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void _changePassword() async {
    ...
    final cubit = context.read<AuthCubit>();
    final success = await cubit.changePassword(newPass);
    if (success && mounted) {
      AppSnackbar.show(
        context,
  ...
  Widget _buildMobileSingleColumnLayout(
    ...
    final horizontalPadding = isTablet ? (constraints.maxWidth - 700) / 2 : 0.0;
