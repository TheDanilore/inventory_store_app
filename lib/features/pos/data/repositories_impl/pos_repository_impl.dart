...
class PosRepositoryImpl implements PosRepository {
  ...
  @override
  Future<Either<Failure, PosInitData>> loadInitialData({bool forceRefresh = false}) async {
    try {
      ...
      final warehouses = (whRes as List).map((w) => WarehouseModel.fromJson(w)).toList();
  ...
  @override
  Future<Either<Failure, CashShiftEntity?>> checkActiveShift(String accountId) async {
    try {
  ...
  @override
  Future<Either<Failure, List<Map<String, dynamic>>>> searchClients(String text) async {
    try {
  ...
  @override
  Future<Either<Failure, Map<String, dynamic>?>> fetchClientCredit(String clientId) async {
    try {
      final response = await _supabase
  ...
  Future<Either<Failure, String>> processSale(SaleEntity sale) async {
    try {
      ...
      for (final item in sale.items) {
        ...
        if (safeVariantId == null) throw Exception('No variant ID for item ${item.productId}');

        List<({String id, int take, int available, String batchNumber})> segments = [];
        ...
        if (!sale.isDraft) {
          for (final seg in segments) {
            ...
            movementInserts.add({
              ...
              'reason': 'SALE',
              'notes': 'Venta POS - ${sale.paymentMethod} • Lote: ${seg.batchNumber}',
      ...
      final orderResp = await _supabase
          ...
          .insert({
            ...
            'customer_name': sale.customerId == null ? sale.customerName : null,
            'warehouse_id': sale.warehouseId,
      ...
      if (!sale.isDraft) {
        if (sale.activeShift != null && sale.accountId != null) {
          ...
          await _supabase.from('financial_accounts').update({
            'balance': currentBalance + sale.amountPaid,
          }).eq('id', sale.accountId!);
        }

        for (final upd in batchUpdates) {
          await _supabase.from('warehouse_stock_batches').update({
            'available_quantity': upd['new_quantity'],
          }).eq('id', upd['id']);
        }
        ...
        if (sale.pointsEarned > 0 || sale.pointsUsed > 0) {
          if (sale.customerId != null) {
            final profileWalletResp = await _supabase
                .from('profiles')
                .select('wallet_balance')
                .eq('id', sale.customerId!)
                .single();
            final currentPoints = (profileWalletResp['wallet_balance'] as num).toInt();
            final newPoints = currentPoints + sale.pointsEarned - sale.pointsUsed;

            await _supabase.from('profiles').update({
              'wallet_balance': newPoints,
            }).eq('id', sale.customerId!);

            await _supabase.from('wallet_movements').insert({
              'profile_id': sale.customerId,
              'amount': sale.pointsEarned - sale.pointsUsed,
              'movement_type': (sale.pointsEarned - sale.pointsUsed) >= 0 ? 'EARNED' : 'REDEEMED',
