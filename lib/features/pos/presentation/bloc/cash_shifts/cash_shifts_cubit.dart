...
class CashShiftsCubit extends Cubit<CashShiftsState> {
  ...
  Future<void> fetchShifts() async {
    ...
    shiftsRes.fold(
      ...
      (data) {
        final openAccountIds = <String>{
          for (final shift in data.shifts)
            if (shift.status == CashShiftStatus.open && shift.accountId != null)
              shift.accountId!,
        };
  ...
  Future<void> closeShift(
    ...
    emit(state.copyWith(isLoading: true, errorMessage: ''));
    final res = await _closeShift(
      CloseCashShiftParams(
        shiftId: shiftId,
  ...
  Future<double> calcExpected(
    String shiftId,
    String accountId,
    double openingAmount,
  ) async {
    final res = await _calcExpected(
