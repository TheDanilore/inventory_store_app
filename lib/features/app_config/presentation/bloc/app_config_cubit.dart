...
class AppConfigCubit extends Cubit<AppConfigState> {
  ...
  Future<void> fetchSettings() async {
    ...
    result.fold(
      (failure) => emit(
        state.copyWith(status: ViewState.error, errorMessage: failure.message),
      ),
      (settings) =>
          emit(state.copyWith(status: ViewState.success, values: settings)),
    );
  }
  ...
  Future<bool> saveBusinessInfo({
    ...
    emit(
      state.copyWith(saveStatus: ViewState.loading, clearErrorMessage: true),
    );
  ...
  Future<void> changeConnection(String url, String key) async {
    ...
    final result = await changeConnectionUseCase(
      ChangeConnectionParams(url: url, key: key),
    );
