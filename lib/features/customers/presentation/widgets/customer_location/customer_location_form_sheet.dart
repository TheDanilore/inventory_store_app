...
class _CustomerLocationFormSheetState extends State<CustomerLocationFormSheet> {
  ...
  Future<void> _pickFromMap() async {
    final initialPoint = (_lat != 0 && _lng != 0) ? LatLng(_lat, _lng) : null;
    final placeResult = await Navigator.of(context).push<PlaceResult?>(
      MaterialPageRoute(
        builder:
            (_) => CustomerLocationMapScreen(
              isPickerMode: true,
              initialPickerPoint: initialPoint,
            ),
      ),
    );
    ...
    if (placeResult != null && mounted) {
      setState(() {
        _lat = placeResult.latitude;
        _lng = placeResult.longitude;
        if (placeResult.fullAddress.isNotEmpty) {
          _addressCtrl.text = placeResult.fullAddress;
        }
      });
    }
  }
