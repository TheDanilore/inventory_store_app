...
class CustomerCreditMovementsCubit extends Cubit<CustomerCreditMovementsState> {
  ...
  CustomerCreditMovementsCubit(this._repository)
      : super(const CustomerCreditMovementsState(
          creditId: '',
          customerName: '',
          currentDebt: 0.0,
          creditLimit: 0.0,
  ...
  Future<void> loadData() async {
    ...
    try {
      final dateFilterParam = state.dateFilter == 'all' ? null : state.dateFilter;
      ...
      final movements = results[0] as List; // In real impl, you'd get total count. We will simulate count or check if list is complete
      ...
      final totalResult = results[1] as ({double totalCharged, double totalPaid});

      int simulatedTotalCount = state.currentPage * state.pageSize + movements.length;
