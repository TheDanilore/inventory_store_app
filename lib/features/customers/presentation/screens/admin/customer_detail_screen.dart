...
import 'package:inventory_store_app/features/customers/presentation/bloc/customer_detail/customer_detail_cubit.dart';
...
class CustomerDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final targetId = customerId.isNotEmpty ? customerId : (customer?.id ?? '');
...
class _CustomerDetailContent extends StatelessWidget {
  ...
  void _openEditCustomer(BuildContext context) async {
    ...
    if (state is CustomerDetailLoaded) {
      final saved = await CustomerFormSheet.show(
        context,
            ...
            delegate: SliverChildListDelegate([
              ...
              BlocBuilder<CustomerLocationsCubit, CustomerLocationsState>(
                builder: (context, locState) {
                  return _buildLocationsSection(context, locState, c.id);
                },
              ),
  ...
  Widget _buildLocationsSection(
    BuildContext context,
    CustomerLocationsState locState,
    String customerId,
  ) {
    ...
    if (locState is CustomerLocationsLoading) {
      return const CustomerSectionCard(
        title: 'Ubicaciones',
        icon: Icons.map_rounded,
        child: AppShimmer(height: 120, borderRadius: 16),
      );
    }
    if (locState is CustomerLocationsError) {
      return CustomerSectionCard(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              Text(
                'Error al cargar ubicaciones: ${locState.message}',
