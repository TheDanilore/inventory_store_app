...
class SupplierCreditsService {
  ...
  Future<({List<SupplierCreditModel> accounts, int count, Map<String, dynamic> stats})> fetchAccountsPaginated({
    required int page,
    ...
    var statsQuery = _supabase.from('supplier_credits').select('*, suppliers!inner(name, tax_id, phone)');
    ...
    var query = _supabase.from('supplier_credits').select('id, supplier_id, credit_limit, current_debt, is_active, suppliers!inner(name, tax_id, phone)');
    ...
    final response = await query.order('current_debt', ascending: false).range(from, to).count(CountOption.exact);
    final count = response.count;
    final list = (response.data as List).map((item) => SupplierCreditModel.fromView(item)).toList();
  ...
  Future<void> toggleAccountStatus(String creditId, bool currentStatus) async {
    await _supabase.from('supplier_credits').update({
  ...
  Future<List<Map<String, dynamic>>> searchSuppliers(String query, Set<String> existingSupplierIds) async {
  ...
  Future<Set<String>> getExistingCreditSupplierIds({String? excludeSupplierId}) async {
    final existingCredits = await _supabase.from('supplier_credits').select('supplier_id');
  ...
  Future<List<Map<String, dynamic>>> getPendingPurchaseOrders(String supplierId) async {
  ...
  Future<List<SupplierFinancialAccountOption>> getFinancialAccounts() async {
    ...
    return (resp as List)
        .map((a) => SupplierFinancialAccountOption(
              id: a['id'] as String,
  ...
  Future<void> registerPayment({
    ...
    final ordersToApply = selectedOrderId != null
