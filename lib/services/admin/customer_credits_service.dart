...
class CustomerCreditsService {
  ...
  Future<({List<CreditAccountModel> accounts, int count, Map<String, dynamic> stats})> fetchAccountsPaginated({
    required int page,
    ...
    final list = (response.data as List).map((item) => CreditAccountModel.fromView(item)).toList();
  ...
  Future<List<Map<String, dynamic>>> searchClients(String query, Set<String> existingProfileIds) async {
    final response = await _supabase
        .from('profiles')
        .select('id, full_name, document_number, document_type, phone')
        .eq('role', 'customer')
        .eq('is_active', true)
        .or('full_name.ilike.%$query%,document_number.ilike.%$query%,phone.ilike.%$query%')
        .limit(20);
  ...
  Future<Set<String>> getExistingCreditProfileIds({String? excludeProfileId}) async {
    final existingCredits = await _supabase.from('customer_credits').select('profile_id');
  ...
  Future<void> registerPayment({
    ...
    for (final order in ordersToApply) {
      ...
      if (pointsEarned > 0) {
        // Obtener billetera
        final profileResp = await _supabase.from('profiles').select('wallet_balance').eq('id', account.profileId).single();
        final currentWallet = (profileResp['wallet_balance'] as num?)?.toInt() ?? 0;
        
        // Sumar y actualizar
        await _supabase.from('profiles').update({
          'wallet_balance': currentWallet + pointsEarned,
