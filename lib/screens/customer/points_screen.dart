...
class _PointsScreenState extends State<PointsScreen> {
  ...
  List<int> _buildMiniGameBoxes(AppConfigProvider config) {
    final prize1 = config.getDouble(_boxesPrize1Key, _defaultBoxesPrize1).toInt();
    final prize2 = config.getDouble(_boxesPrize2Key, _defaultBoxesPrize2).toInt();
    final prize3 = config.getDouble(_boxesPrize3Key, _defaultBoxesPrize3).toInt();
  ...
  Future<void> _fetchPointsData() async {
    ...
    try {
      final config      = context.read<AppConfigProvider>();
      final reward      = config.getDouble('checkin_reward',
          config.getDouble('daily_checkin_reward', 10)).round();
      ...
      final todayCheckin = await _supabase.from('daily_checkins')
          .select('id').eq('profile_id', profileId).eq('checkin_date', todayDate)
          .maybeSingle();

      final latestCheckin = await _supabase.from('daily_checkins')
          .select('checkin_date, streak_day, points_received, created_at')
          .eq('profile_id', profileId).order('checkin_date', ascending: false)
          .limit(1).maybeSingle();
      ...
      final todayGames = List<Map<String, dynamic>>.from(
        await _supabase.from('wallet_movements')
      ...
      final boxGame = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_BOXES')
          .cast<Map<String, dynamic>?>().firstWhere((_) => true, orElse: () => null);

      final movements = await _supabase.from('wallet_movements')
          .select('*, orders(id)').eq('profile_id', profileId)
      ...
      setState(() {
        ...
        _boxesPlaysToday      = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_BOXES').length;
        _memoramaPlaysToday   = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_MEMORY').length;
        _catcherPlaysToday    = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_CATCHER').length;
        _pinataPlaysToday     = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_PINATA').length;
        _superSaltoPlaysToday = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_JUMP').length;
        _clawPlaysToday       = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_CLAW').length;
        _stackPlaysToday      = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_STACK').length;
        _dodgePlaysToday      = todayGames.where((g) => g['movement_type'] == 'MINI_GAME_DODGE').length;
        _lastCheckinDate      = latestCheckinDate;
        _lastBoxesReward      = boxGame == null ? null : (boxGame['points'] as num?)?.toInt();
      ...
      AppSnackbar.show(context, message: 'Error cargando monedas: $e', type: SnackbarType.error);
    }
  }
  ...
  Future<void> _claimDailyCheckin() async {
    ...
    final nextStreakDay = _lastCheckinDate != null && isSameDay(_lastCheckinDate!, yesterday)
        ? _currentStreak + 1
        : 1;
    ...
    try {
      ...
      if (!mounted) return;
      AppSnackbar.show(context,
          message: e.code == '23505' ? 'Ya reclamaste tus monedas de hoy.'
              : 'No se pudo reclamar el check-in: ${e.message}',
          type: SnackbarType.error);
    } catch (e) {
      if (!mounted) return;
      AppSnackbar.show(context,
          message: 'No se pudo reclamar el check-in: $e', type: SnackbarType.error);
    } finally {
  ...
  Future<void> _playBoxMiniGame(int boxIndex) async {
    ...
    try {
      ...
      if (!mounted) return;
      AppSnackbar.show(context,
          message: 'No se pudo jugar en este momento: $e', type: SnackbarType.error);
    } finally {
...
Future<void> _openCoinCatcherGame() async {
    final limit = context.read<AppConfigProvider>().getDouble('catcher_daily_limit', 1).round();
    if (_profileId == null || _catcherPlaysToday >= limit) return;
    final reward = await Navigator.push<int>(
      context, MaterialPageRoute(builder: (_) => CoinCatcherGameScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (reward != null) { 
      setState(() => _catcherPlaysToday += 1); 
      await _fetchPointsData(); 
      if (mounted) Provider.of<WalletProvider>(context, listen: false).refresh(); // AVISO
    }
  }
  ...
  Future<void> _openMemoramaGame() async {
    ...
    final reward = await Navigator.push<int>(
      context, MaterialPageRoute(builder: (_) => MemoramaGameScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (reward != null) { 
  ...
  Future<void> _openPinataGame() async {
    final limit = context.read<AppConfigProvider>().getDouble('pinata_daily_limit', 1).round();
    if (_profileId == null || _pinataPlaysToday >= limit) return;
    final reward = await Navigator.push<int>(
      context, MaterialPageRoute(builder: (_) => PinataGameScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (reward != null) { 
  ...
  Future<void> _openSuperSaltoGame() async {
    ...
    final pts = await Navigator.push<int>(
      context, MaterialPageRoute(builder: (_) => SuperSaltoScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (pts != null) { 
  ...
  Future<void> _openClawMachineGame() async {
    final limit = context.read<AppConfigProvider>().getDouble('claw_daily_limit', 1).round();
    if (_profileId == null || _clawPlaysToday >= limit) return;
    final r = await Navigator.push<int>(
      context, MaterialPageRoute(builder: (_) => ClawMachineScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (r != null) { 
  ...
  Future<void> _openStackGame() async {
    final limit = context.read<AppConfigProvider>().getDouble('stack_daily_limit', 1).round();
    if (_profileId == null || _stackPlaysToday >= limit) return;
    final r = await Navigator.push<int?>(
      context, MaterialPageRoute(builder: (_) => StackGameScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (r != null) {
      await _fetchPointsData();
      if (mounted) Provider.of<WalletProvider>(context, listen: false).refresh(); // AVISO
  ...
  Future<void> _openDodgeGame() async {
    final limit = context.read<AppConfigProvider>().getDouble('dodge_daily_limit', 1).round();
    if (_profileId == null || _dodgePlaysToday >= limit) return;
    final r = await Navigator.push<int?>(
      context, MaterialPageRoute(builder: (_) => DodgeGameScreen(profileId: _profileId!)));
    if (!mounted) return;
    if (r != null) {
      await _fetchPointsData();
      if (mounted) Provider.of<WalletProvider>(context, listen: false).refresh(); // AVISO
  ...
  Widget build(BuildContext context) {
    ...
    final claimMessage = _hasTodayCheckin
    ...
    final prizePreview = _boxesRoundReady && _miniGameBoxes.isNotEmpty
        ? _miniGameBoxes
        : (_miniGamePreviewBoxes.isNotEmpty ? _miniGamePreviewBoxes : _miniGameBoxes);
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Stack(
        children: [
          // Decorative circles
          Positioned(right: -20, top: -20, child: _DecorCircle(size: 110, opacity: 0.06)),
          Positioned(right: 30, bottom: -30, child: _DecorCircle(size: 80, opacity: 0.05)),
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  const SizedBox(width: 10),
                  Text('Llevas $currentStreak días seguidos reclamando.',
                    style: const TextStyle(
          ...
          GestureDetector(
            ...
            child: AnimatedContainer(
              ...
              decoration: BoxDecoration(
                ...
                borderRadius: BorderRadius.circular(_DS.radius),
                boxShadow  : hasTodayCheckin ? null : [
                  BoxShadow(
              ...
              child: Row(
                ...
                children: [
                  if (isClaimingCheckin)
                    ...
                    Icon(
                      hasTodayCheckin ? Icons.check_circle_outline : Icons.touch_app_rounded,
                      color: hasTodayCheckin ? _DS.textMuted : Colors.white,
                      size : 18,
                    ),
                  const SizedBox(width: 8),
                  Text(
                    isClaimingCheckin ? 'Reclamando…'
                        : hasTodayCheckin  ? 'Ya reclamado hoy'
                        : 'Reclamar monedas',
                    style: TextStyle(
                      fontSize  : 14,
                      fontWeight: FontWeight.w800,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Diviértete y gana monedas extra',
          ...
          _buildGameTile(title: 'Memorama',        emoji: '🧠', plays: playsToday,          limit: dailyLimit,       color: _DS.teal,                  onPlay: canPlay          ? onPlayMemorama    : null),
          _buildGameTile(title: 'Lluvia de Monedas',emoji: '🪙', plays: catcherPlaysToday,   limit: catcherDailyLimit,color: const Color(0xFFE5A93C),   onPlay: canPlayCatcher   ? onPlayCoinCatcher : null),
          _buildGameTile(title: 'Piñata',           emoji: '🎉', plays: pinataPlaysToday,    limit: pinataDailyLimit, color: const Color(0xFFE05C41),   onPlay: canPlayPinata    ? onPlayPinata      : null),
          _buildGameTile(title: 'Máquina de Garra', emoji: '🦾', plays: clawPlaysToday,      limit: clawDailyLimit,   color: const Color(0xFFB26CFF),   onPlay: canPlayClaw      ? onPlayClaw        : null),
          _buildGameTile(title: 'Torre de Cajas',   emoji: '📦', plays: stackPlaysToday,     limit: stackDailyLimit,  color: const Color(0xFF4E79FF),   onPlay: canPlayStack     ? onPlayStack       : null),
          _buildGameTile(title: 'Esquiva y Atrapa', emoji: '⚡', plays: dodgePlaysToday,     limit: dodgeDailyLimit,  color: const Color(0xFF3E7DD1),   onPlay: canPlayDodge     ? onPlayDodge       : null),
          _buildGameTile(title: 'Super Salto',      emoji: '🚀', plays: superSaltoPlaysToday,limit: superSaltoDailyLimit,color: const Color(0xFF6A5AE0), onPlay: canPlaySuperSalto? onPlaySuperSalto  : null),
        ],
      ),
    );
  }
      ...
      child: Row(
        children: [
          ...
          Container(
            width : 44, height: 44,
            decoration: BoxDecoration(
              color       : active ? color.withValues(alpha: 0.12) : const Color(0xFFE2E8F0),
              borderRadius: BorderRadius.circular(12),
            ),
            child: Center(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 3),
                Text('$plays / $limit juego${limit != 1 ? "s" : ""} hoy',
                  style: TextStyle(
                    fontSize  : 10,
                    color     : active ? color : _DS.textMuted,
                    fontWeight: FontWeight.w600,
...
class PointsMiniGameCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final (headerBg, headerIcon, headerIconColor) = phase == 'done'
        ...
        children: [
              ...
              const Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Elige la caja con el mejor premio',
                      style: TextStyle(fontSize: 11, color: _DS.textMuted),
                    ),
                  ],
                ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color       : phase == 'pick'
                  ? _DS.goldLight
                  : phase == 'done' ? const Color(0xFFF1F5F9) : _DS.tealLight,
              borderRadius: BorderRadius.circular(10),
            ),
            child: Row(
              children: [
                Icon(
                  phase == 'pick' ? Icons.touch_app_rounded
                      : phase == 'done' ? Icons.check_circle_rounded
                      : Icons.info_outline_rounded,
                  size : 14,
                  color: phase == 'pick' ? _DS.goldDark
                      : phase == 'done' ? _DS.textMuted : _DS.teal,
                ),
                ...
                Expanded(
                  child: Text(phaseLabel,
                    style: TextStyle(
                      ...
                      fontWeight: FontWeight.w600,
                      color     : phase == 'pick' ? _DS.goldDark
                          : phase == 'done' ? _DS.textMuted : _DS.tealDark,
                    ),
                  ),
                ),
              ],
          ...
          Row(
            children: List.generate(prizePreview.length, (index) {
              ...
              final wobble     = isPreparingBoxes ? ((shuffleSeed + index) % 3 - 1) * 0.04 : 0.0;
              final vertShift  = isPreparingBoxes ? ((shuffleSeed + index * 2) % 3 - 1) * 5.0 : 0.0;
                      ...
                      child: Transform.rotate(
                        ...
                        child: AnimatedContainer(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(
                                showPreviewValues ? '🎁' : locked ? '🔒' : '📦',
                                style: const TextStyle(fontSize: 26),
                              ),
                              const SizedBox(height: 6),
                              Text(
                                showPreviewValues ? '+$reward'
                                    : locked ? (canPlay ? 'Caja ${index + 1}' : '—')
                                    : 'Caja ${index + 1}',
                                textAlign: TextAlign.center,
                                style: TextStyle(
                                  color     : locked ? _DS.textMuted : const Color(0xFF7A4500),
                                  fontWeight: FontWeight.w900,
              ...
              child: Row(
                ...
                children: [
                  if (isPlayingMiniGame)
                    ...
                    Text(canPlay ? '🎲' : '✅', style: const TextStyle(fontSize: 16)),
                  const SizedBox(width: 8),
                  Text(
        ...
        child: Column(
          children: [
            Container(
              ...
              child: const Icon(Icons.history_rounded, size: 28, color: _DS.textMuted),
            ),
            const SizedBox(height: 12),
            ...
            const SizedBox(height: 4),
            const Text('Tu historial de monedas aparecerá aquí',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          ...List.generate(visible, (i) => _MovementRow(movement: movements[i])),
          ...
          if (movements.length > 10) ...[
            ...
            Container(
              ...
              child: Center(
                child: Text('Ver ${movements.length - 10} movimientos más',
                  style: const TextStyle(
                    fontSize  : 12,
                    fontWeight: FontWeight.w700,
...
class _MovementRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    DateTime? parsedDate;
    try { parsedDate = DateTime.parse(movement['created_at'].toString()).toLocal(); } catch (_) {}

    final (emoji, badgeColor, badgeBg) = _typeMap[type]
        ?? (isPositive ? ('⬆', _DS.success, _DS.successLight)
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                if (parsedDate != null) ...[
                  const SizedBox(height: 2),
                  Text(DateFormat('dd MMM · HH:mm').format(parsedDate),
...
class PointsHeroSection extends StatelessWidget {
  final int    currentBalance;
  final String hundredCoinsValue;
  const PointsHeroSection({super.key, required this.currentBalance, required this.hundredCoinsValue});
...
class PointsBalanceCard extends StatelessWidget {
  final int    currentBalance;
  final String hundredCoinsValue;
  const PointsBalanceCard({super.key, required this.currentBalance, required this.hundredCoinsValue});
